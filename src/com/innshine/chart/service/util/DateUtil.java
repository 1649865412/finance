...
public class DateUtil
{
	...
	// 求时间分析x轴(Z季)
	public static String[] xValueZ(String firstTime, String endTime)
			throws ParseException
	{
		...
		String[] array = Constant.Z_MONTH[firstJidu - 1];
		for (int i = firstJidu; i < lastJidu; i++)
		{
			array = SqlChangetool.concat(array, Constant.Z_MONTH[i]);
		}
		return array;
	}
	...
	public static String changeMoneyType(Double num)
	{
		...
		if (fm1.format(num).equals(".00"))
			...
			resultnum = num.toString().substring(0, 3);
		else
